...
def fillMissingValues(missing_dataframe):
    ...
    assembly_codes = pd.read_csv("IBM_assembly_code.csv", usecols=["Name", "Assembly Code"], encoding="unicode_escape")
...
def bulkUpdateDB(udpate_df):
    ...
    try:
        for _, row in udpate_df.iterrows():
            updates.append(UpdateOne({'model_id': row.get('model_id')}, {'$set': {'Assembly_Code': row.get('Assembly_Code'), 'ML_filled' : 1}}, upsert=True))
...
def updatedb(projectId):
    ...
    print('> No of updated assembly codes udpated using ML model : ',len(filtered_df))
    ...
    # return succes if update is success else return false
